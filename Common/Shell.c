...
static void CDC_StdIOSendChar(uint8_t ch) {
	while (CDC1_SendChar((uint8_t) ch) == ERR_TXFULL) {
	} /* Send char */
}
...
#endif
#if PL_HAS_USB_CDC
		(void) CLS1_ReadAndParseWithCommandTable(cdc_buf, sizeof(cdc_buf),
				&CDC_stdio, CmdParserTable);
#endif
#if PL_HAS_BLUETOOTH
...
#endif
#if PL_HAS_RTOS
	if (FRTOS1_xTaskCreate(ShellTask, "Shell", configMINIMAL_STACK_SIZE+100,
